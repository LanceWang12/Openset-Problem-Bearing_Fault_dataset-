...
class bearing_dataset(Dataset):
    def __init__(
        ...
        length = 0
        DB = [[] for _ in range(len(class_lst))]
        total_lst = []
        label_lst = []
    ...
    def __getitem__(self, index: int):
        if self.train:
            ...
            anchor, positive = choice(self.DB[class1], size = 2, replace = False)
            anchor, positive = torch.as_tensor(np.load(anchor), dtype = torch.float32), torch.as_tensor(np.load(positive), dtype = torch.float32)
            negative = torch.as_tensor(np.load(choice(self.DB[class2], size = 1)[0]), dtype = torch.float32)
